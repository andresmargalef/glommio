...
pub(crate) mod test {
    ...
    dma_file_test!(per_queue_stats, path, _k, {
        ...
        let q2 = Local::create_task_queue(Shares::default(), Latency::NotImportant, "q2");
        let task1 =
        ...
        let stats = Local::io_stats();
        assert_eq!(stats.file_opened(), 4);
        assert_eq!(stats.file_closed(), 4);
        assert_eq!(stats.file_reads(), (4, 9216));
        assert_eq!(stats.file_writes(), (2, 8192));
